...
def main(args):
  ...
  for line in open(args.facescrub_lst, 'r'):
    ...
    a, b = _path[-2], _path[-1]
    feature_path = os.path.join(args.facescrub_feature_dir, a, "%s%s"%(b, args.suffix))
    ...
    feature_path_out = os.path.join(args.facescrub_feature_dir_out, a, "%s%s"%(b, out_algo))
    #print(b)
  ...
  for line in open(args.megaface_lst, 'r'):
    ...
    if not os.path.exists(feature_dir_out):
      os.makedirs(feature_dir_out)
    feature_path_out = os.path.join(args.megaface_feature_dir_out, a1, a2, "%s%s"%(b, out_algo))
    bb = '/'.join([a1, a2, b])
    #print(b)
...
def parse_arguments(argv):
  ...
  parser.add_argument('--facescrub-noises', type=str, help='', default='./facescrub_noises.csv')
  parser.add_argument('--megaface-noises', type=str, help='', default='./megaface_noises.csv')
  parser.add_argument('--suffix', type=str, help='', default='.bin')
  ...
  parser.add_argument('--facescrub-feature-dir-out', type=str, help='', default='/home/brl/Megaface/cleanedFeature/facescrub')
